            ...
            {Object.entries(insertExamples).map(([key, content], index) => (
              <section key={key} className="bg-white border border-stone-900/[0.08] rounded-[18px] p-7 mb-8 shadow-sm">
                <div className="flex items-center gap-3.5 mb-4">
                  ...
                  <h2 className="font-display text-[19px] font-bold text-stone-900">{content.title}</h2>
                </div>
                ...
                <div className="rounded-[13px] overflow-hidden bg-[#0E1117] border border-white/[0.06]">
                  <div className="flex items-center justify-between px-3.5 py-2.5 bg-[#161B22]">
                    ...
                    <button onClick={() => handleCopy(content[activeTab])} className="font-mono text-[10px] text-white/35 bg-transparent border-none cursor-pointer">
                      {copied ? "✓ Copied" : "Copy"}
                    </button>
                  </div>
